...
function createContent(summaryData) {
	...
	summaryContent = "<b> <font color='brown'>Summary :</font></b>&nbsp;&nbsp;&nbsp;&nbsp; <b> Vending Machine : </b><font color='blue'>"
		+ summaryData.summary.vendingmachine
		+ "</font>&nbsp;&nbsp;&nbsp;&nbsp;<b> Total Sales : <font color='blue'>"
		+ summaryData.summary.totalSales + "</font>";
...
// https://dojotoolkit.org/reference-guide/1.8/dojox/charting.html
	function showDojoChart() {
		...
		var chartData = "{\"identifier\":\"entryindex\",\"label\":\"Sales\",\"items\":[";
		var chartData2 = [];
		//
		for (var i = 0; i < chartdetails.length; i++) {
			...
			// Y -Axis Value
			console.log("Sales  : " + entry.expense);
			chartData = chartData + "{" + "\"entryindex\": " + i + ", \"sales\": "
				+ entry.expense + "}";
			chartData2[i] = entry.expense;
		...
		if (!chart) {
			chart = new dojox.charting.Chart("chartNode", {
				title: "Vending Machine Sales Chart",
				titlePos: "top",
				titleGap: 25,
			...
			chart.addSeries("Vending Machine Sales ", chartData2);
			// chart.setStore(mystore, {entryindex:"*"}, "price");
			console.log("Rendering the Chart...");
		} else {
			chart.updateSeries("Vending Machine Sales ", chartData2);
		}
		// tooltips!
			...
			return function () {
				if (!tickersummary) {
					tickersummary = dojo.xhrGet({
						url: "/vendingmachinesummary/" + symbol,
						handleAs: "json"
					});
				}
	...
	dojo.addOnLoad(function () {
		...
		socket.on("message",
			function (msg) {
				...
				dojo.byId("stocks-symbols").innerHTML = "<div class='stocks-data'>" + "<div class='stocks-symbol'><a class='stocks-symb-link' href='#' data='" + data.vendingmachine + "' title='Payrange Payment Analysis'>" + data.vendingmachine + "</a></div>" +
					"<div class='stocks-meta stocks-price'>$" + data.expense+", "+data.product+ "</div>" +
					"<div class='stocks-meta stocks-volume'>" + data.user + ", "+data.city +"</div>" +
					"</div>" + html;
